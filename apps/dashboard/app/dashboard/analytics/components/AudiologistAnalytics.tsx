...
import { useGetAudiologistMetrics } from "@/hooks/consultation/use-get-audiologist-anayltics";
...
const filterSchema = z.object({
  metricType: z.enum(["audiologists"]),
...
interface AudiologistAnalyticsProps {
  userRole?: string;
  currentUser?: any;
}
...
export default function AudiologistAnalytics({ userRole, currentUser }: AudiologistAnalyticsProps) {
  ...
  const isNormalAudiologist = userRole === "AUDIOLOGIST";
  
  const form = useForm<FilterFormData>({
    resolver: zodResolver(filterSchema),
    defaultValues: {
      metricType: "audiologists",
      aggregation: "count",
      groupBy: "day",
      timeRange: "daily",
      useCustomDate: false,
      ...
      endDate: new Date(),
      centreIds: [],
      // For normal audiologists, filter by their own ID
      audiologistIds: isNormalAudiologist && currentUser?.id ? [currentUser.id] : [],
      cityIds: [],
      stateIds: [],
  ...
  const requestBody = {
    ...
    filters: {
      ...
      // Enforce role-based audiologist filtering
      audiologistIds: isNormalAudiologist && currentUser?.id 
        ? [currentUser.id] // Force normal audiologists to only see their own data
        : watchedValues.audiologistIds || [],
  ...
  const FilterButton = (
    <div className="flex gap-2">
      {/* Only show filters for admin and head audiologist */}
      {(isAdmin || isHeadAudiologist) && (
        <Button
          variant="outline"
          size="sm"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Audiologist Analytics</h3>
          <p className="text-sm text-gray-500">
            {isAdmin || isHeadAudiologist 
              ? "View all audiologist metrics and performance" 
      ...
      {/* Enhanced Filters Section - Only for admin and head audiologist */}
      {showFilters && (isAdmin || isHeadAudiologist) && (
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>
                            <SelectItem value="audiologists">🩺 Audiologists</SelectItem>
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
        ...
        <Card className="border-2 border-red-200 bg-red-50">
          <CardContent className="pt-12 pb-12">
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-red-600">There was an error fetching audiologist metrics. Please try again.</p>
              <Button 
                onClick={() => refetch()} 
          ...
          <Card className="shadow-lg">
            <CardHeader className="bg-gradient-to-r from-purple-600 to-blue-600 text-white">
              <CardTitle className="flex items-center space-x-2">
                ...
                <span>Audiologist Metrics Over Time</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-6">
