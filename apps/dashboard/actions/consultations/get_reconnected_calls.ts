...
/**
 * GET /consultation/reconnected-calls
 * Retrieves reconnected calls. Paginated list with patient, centre, audiologist, reconnectedByConsultationId.
 * Supports limit, offset, sortBy, sortOrder, search, startDate, endDate, patientId, audiologistId, centreId, etc.
 */
export interface GetReconnectedCallsParams {
  limit?: number;
  offset?: number;
...
export interface GetReconnectedCallsResult {
  consultations: ConsultationModelData[];
  total: number;
  hasNext: boolean;
}

export default async function getReconnectedCalls(
  params: GetReconnectedCallsParams = {}
): Promise<GetReconnectedCallsResult> {
  ...
  const url = `${baseUrl}consultation/reconnected-calls?${searchParams.toString()}`;
  ...
  if (!res.ok) {
    throw new Error(`Failed to load reconnected calls: ${res.status} ${res.statusText}`);
  }
